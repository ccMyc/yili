...
class OrderPlaceView(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        user = request.user
        #获取参数sku_ids
        sku_ids = request.POST.getlist('sku_ids')  #必须是getlist形式，get的话你只能拿到一个值
...
class OrderCommitView(View):
    ...
    def post(self,request):
        ...
        #todo:清除用户购物车中对应的记录
        #sku_ids:[1,2,3]
        #*sku_ids  可为我们拆包成-》conn.hdel(cart_key,1,2,3)
        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):
                    ...
                    ##update df_goods_sku set stock=new_stork,sales=new_sales
                    #where id=sku_id and stork=orgin_stork
                    #返回值为0 | 1
                    ...
                    #如果用户（查看时的库存==更新时的库存）一次执行成功，则跳出循环
                    break
...
class CommentView(LoginRequiredMixin, View):
    ...
    def post(self, request, order_id):
        ...
        return HttpResponseRedirect(reverse("user:order", kwargs={"page": 1}))
